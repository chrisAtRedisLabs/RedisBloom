...
int TDigestSketch_Merge(RedisModuleCtx *ctx, RedisModuleString **argv, int argc) {
    ...
    if (RedisModule_KeyType(keyTo) == REDISMODULE_KEYTYPE_EMPTY) {
        RedisModule_CloseKey(keyTo);
        return RedisModule_ReplyWithError(ctx, "ERR T-Digest: to key does not exist");
    } else if (RedisModule_ModuleTypeGetType(keyTo) != TDigestSketchType) {
        RedisModule_CloseKey(keyTo);
        return RedisModule_ReplyWithError(ctx, REDISMODULE_ERRORMSG_WRONGTYPE);
    }
    ...
    if (RedisModule_KeyType(keyFrom) == REDISMODULE_KEYTYPE_EMPTY) {
        RedisModule_CloseKey(keyTo);
        RedisModule_CloseKey(keyFrom);
        return RedisModule_ReplyWithError(ctx, "ERR T-Digest: from key does not exist");
    } else if (RedisModule_ModuleTypeGetType(keyFrom) != TDigestSketchType) {
        RedisModule_CloseKey(keyTo);
        RedisModule_CloseKey(keyFrom);
        return RedisModule_ReplyWithError(ctx, REDISMODULE_ERRORMSG_WRONGTYPE);
    }
...
int TDigestSketch_Info(RedisModuleCtx *ctx, RedisModuleString **argv, int argc) {
    ...
    RedisModule_ReplyWithLongLong(ctx, td_compression(tdigest));
